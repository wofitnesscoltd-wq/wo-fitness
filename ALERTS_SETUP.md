...
### 🎯 面板即時止損/止盈建議 ＋ 非洗版 Telegram（唯讀，只建議不下單）
網頁 💼 持股（加密永續）每個倉位會**隨時**顯示一行建議數字：**🛑止損 / 🎯止盈 /（獲利時）⤴上移止損**。
- 全部**本機算、即時、不花 API**：止損用 **ATR＋近20根結構**、依**動能(魚身🟢/轉弱🟡/魚尾🔴)**收緊、並對照你**真實爆倉價**（止損若比爆倉還遠＝會先爆才停損→提示降槓桿）；上移用 **3×ATR Chandelier**（先保本再沿勢推），止盈預設 **2R**。
- 按倉位上的 **套用** 會把數字填進止盈/止損欄，**最後一鍵還是你按「設定」**（本軟體記錄＋到價通知，非交易所下單）。
- **Telegram 只在「需要改設定」時推**（可上移止損鎖利／逼近止損／動能竭盡／止損穿爆倉），同標的至少間隔 10 分鐘、同理由去重，**不洗版**。網頁的 💼 持股頂部有 **🔔 止損/止盈建議推播 開/關**。
- 需求：橋接 ≥ `2026.06.30`（會在 `/health` 回報 `notify`）。舊版橋接前端自動略過、不會報錯。Telegram 走橋接既有金鑰（`.alert_config.json` 或環境變數），**金鑰不進瀏覽器**。
